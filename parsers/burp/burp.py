            ...
            for element in issue:
                if element.tag == "name":
                    ...
                    if name.startswith("Vulnerable version of the library"):
                        ...
                        p = name.find("'")
                        q = name.find("'", p + 1)
                        assert "details" not in data["issues"][0], (
                        ...
                        sys.stderr.write(
                            "Skipping unsupported Burp issue: '%s'\n" % element.text
                        )
                        template_name = None
                ...
                #     could be a false positive.
                    ...
                    if (
                        ...
                        and name != "Vulnerable JavaScript dependency"
                    ):
                    ...
                    assert element.tag not in data["issues"][0], (
                        "Duplicated <%s> tag" % element.tag
                    )
                    data["issues"][0][element.tag] = element.text

                elif element.tag == "host":
                    assert "host" not in data["issues"][0], (
                        "Duplicated <%s> tag" % element.tag
                    )
                    ...
                    if method:
                        assert "method" not in data["issues"][0], (
                            "Duplicated <%s> tag, processing issue '%s'"
                            % (element.tag, name)
                        )
                    ...
                    if (
                        name == "Detailed Error Messages Revealed" and response
                        ...
                        if status == "200":
                            ...
                            headers = dict(
                                [
                                    (h[0].lower().strip(), h[1].lower().strip())
                                    for h in headers
                                ]
                            )
                            if "content-type" in headers:
                                ...
                                ) or headers["content-type"].startswith(
                                    "text/javascript"
                                ):
                                    sys.stderr.write(
                                ...
                                path = data["issues"][0].get(
                                    "path", data["issues"][0].get("location", "")
                                )
                                if "?" in path:
                                    path = path.split("?", 1)[0]
                    ...
                    if name == "Detailed Error Messages Revealed":
                        ...
                        assert " exceptions occur." in element.text, (
                            "Malformed issue: '%s'" % name
                        )
                        ...
                        for li in html.findall(".//li"):
                            ...
                            details.append(
                                fix_header_name(li.find(".//b").text.strip())
                            )
                    ...
                    # Since this may require online access, I'm not sure where to put it.
                        ...
                        result, taxonomy, references = parse_vulners_plugin(
                            element.text
                        )
            ...
            if "vulnerabilities" in issue:
                if issue["vulnerabilities"]:
                    issue["vulnerabilities"] = sorted(
                        ...
                        key=lambda k: k["software"] + "-" + k["version"],
                    )
                else:
...
RE_VULNERS = re.compile(
    "The following vulnerabilities for software <b>([^ ]+) - ([^<]+)</b> found:"
)
        ...
        if href.startswith("https://vulners.com/"):
            ...
            if tax:
                ...
                if found:
                    ...
                    sys.stderr.write(
                        "Unknown taxonomy tag used by Vulners plugin: '%s'\n" % tax
                    )
                    references.append(href)
            else:
                sys.stderr.write(
                    "Unknown Vulners link used by Vulners plugin: '%s'\n" % href
                )
                references.append(href)
        else:
            references.append(href)
