...
TEMPLATES = {
    ...
    86328: "multiple_ssl_issues",   # SSH Diffie-Hellman Modulus <= 1024 Bits (Logjam)
...
def do_generic_nessus_vulnerability(obj):
    ...
    return {
        "template": "generic_nessus_vulnerability",
        "nessus": [vuln]
    }
...
re_default_password = re.compile(r"^Default Password \('([^']+)'\) for '(^']+)' Account$")
...
nessus2testssl = {
    ...
    86328: "LOGJAM",                # SSH Diffie-Hellman Modulus <= 1024 Bits (Logjam)
    ...
    if plugin_id in nessus2testssl and nessus2testssl[plugin_id]:
        ...
        if isinstance(nessus2testssl[plugin_id], str):
            return {
                "hosts": [{
                    "host": host,
                    "problems": {nessus2testssl[plugin_id]: ""}
                }]
            }
        return {
            "hosts": [{
                "host": host,
                "problems": {x:"" for x in nessus2testssl[plugin_id]}
            }]
        }
    return do_generic_nessus_vulnerability(obj)     # XXX TODO
    ...
    for report_host in root.findall(".//ReportHost"):
        ...
        for report_item in report_host.findall("./ReportItem"):
            ...
            try:
                ...
                severity = ["none", "low", "medium", "high", "critical"][int(report_item.attrib["severity"])]
                ...
                if cpe is not None:
                    item["cpe"] = cpe.text
                taxonomy = []
                references = ["https://www.tenable.com/plugins/nessus/" + str(plugin_id)]
                ...
                sys.stderr.write("Error parsing vulnerability %d: %s\n" % (plugin_id, name))
                traceback.print_exc()
                sys.stderr.write("\n")
...
def main():
    ...
    for report_item in nessus_report:
        ...
        plugin_name = report_item["plugin_name"]
