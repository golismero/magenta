...
class Merger:
    ...
    def run(self):
        ...
        while index < len(input_array):
            ...
            try:
                ...
                assert (
                    issue.get("template", self.template_name) == self.template_name
                ), "Wrong issue '%s' sent to merger for '%s'" % (
                    issue["template"],
                    self.template_name,
                )
    ...
    def do_template_init(self):
        return self.template_name

    def do_tools_init(self):
        return []

    def do_severity_init(self):
        return "none"

    def do_affects_init(self):
        return []

    def do_taxonomy_init(self):
        return []

    def do_references_init(self):
        return []

    def do_template_collect(self, merged, issue):
        return self.template_name

    def do_severity_collect(self, merged, issue):
        return (
            issue
            if SEVERITY_LABELS.index(issue) > SEVERITY_LABELS.index(merged)
            else merged
        )

    def do_template_cleanup(self, value):
        return value
